...
class system(win.window):
	...
	def load_initial_condition(self, x, y, z): # only one phase:  everything's square!
		...
		X[2::model.N_EQ1] = self.z_orbit([0., phi_x, phi_y, phi_z])
		return X
	...
	def load_initial_conditions(self, initial_phase): # only one phase:  everything's square!
		...
		for i in xrange(n):
			...
			for j in xrange(n):
				x_j, y_j, z_j = self.x_orbit(phi[j]), self.y_orbit(phi[j]), self.z_orbit(phi[j])

				for k in xrange(n):
					X[i*n2+j*n+k, model.N_EQ1:] = np.array([x_i, y_i, z_i, x_j, y_j, z_j, self.x_orbit(phi[k]), self.y_orbit(phi[k]), self.z_orbit(phi[k])])
